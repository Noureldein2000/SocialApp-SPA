...
export class PhotoEditorComponent implements OnInit {
  ...
  @Input() photos: Photo[];
  @Output() getMemberMainPhoto = new EventEmitter();
  ...
  intializeUploader() {
    this.uploader = new FileUploader({
      url: this.baseUrl + 'users/' + this.authService.decodedToken.nameid + '/photos/AddphotoForUser',
  ...
  setMainPhoto(photo: Photo) {
    this.userService.setMainPhoto(this.authService.decodedToken.nameid, photo.id).subscribe(resp => {
      ...
      this.authService.currentUser.photoUrl = photo.url;
      localStorage.setItem('user', JSON.stringify(this.authService.currentUser));
      this.alertify.success("Success to set main..!");
    },
      err => {
        console.log('Failure set to main about ' + err);
        this.alertify.error("Failure to set main..!");
      })
  }
  ...
  deletePhoto(id: number) {
    this.alertify.confirm('Are you sure you want to delete?', () => {
      this.userService.deletePhoto(this.authService.decodedToken.nameid, id).subscribe(resp => {
        ...
        err => {
          console.log('error deleting is ' + err);
          this.alertify.error('Failure to delete this photo');
        });
    });
  }
}
